...
class Validation
{
     ...
     * @return void
     */
    public function setErrMsg(array $array)
    {
        foreach ($this->errorMsgs as $key => $value) {
            if (isset($array[$key])) {
                $this->errorMsgs[$key] = $array[$key];
            }
        }
    }
     ...
     * @return void
     */
    protected function getErrMsg(string $name, array $replace)
    {
        $msg = $this->errorMsgs[$name] ?? null;
    ...
    public function pattern($name)
    {
        ...
        if ($name == 'array') {
            ...
            if (!is_array($this->value)) {
                $this->errors[$this->name][] = $this->getErrMsg("pattern", ["name" => $this->name]);
            }
        } else {
    ...
    public function required()
    {
        ...
        if ((isset($this->file) && $this->file['error'] == 4) || ($this->value == '' || $this->value == null)) {
            $this->errors[$this->name][] = $this->getErrMsg("required", ["name" => $this->name]);
        }
        return $this;
    }
    ...
    public function min($length)
    {
        ...
        if (is_string($this->value)) {
            ...
            if (strlen($this->value) < $length) {
                $this->errors[$this->name][] = $this->getErrMsg("min", ["name" => $this->name]);
            }
        } else {
    ...
    public function max($length)
    {
        ...
        if (is_string($this->value)) {
            ...
            if (strlen($this->value) > $length) {
                $this->errors[$this->name][] = $this->getErrMsg("max", ["name" => $this->name]);
            }
        } else {
    ...
    public function equal($value)
    {
        ...
        if ($this->value != $value) {
            $this->errors[$this->name][] = $this->getErrMsg("equal", ["name" => $this->name]);
        }
        return $this;
    }
    ...
    public function maxSize($size)
    {
        ...
        if ($this->file['error'] != 4 && $this->file['size'] > $size) {
            $maxSize = number_format($size / 1048576, 2);

            $this->errors[$this->name][] =
                $this->getErrMsg("maxSize", ["name" => $this->name, "filesize" => $maxSize]);
        }
        return $this;
    }
    ...
    public function ext($extension)
    {
        ...
        if ($this->file['error'] != 4 && pathinfo($this->file['name'], PATHINFO_EXTENSION) != $extension && strtoupper(pathinfo($this->file['name'], PATHINFO_EXTENSION)) != $extension) {
            $this->errors[$this->name][] =
                $this->getErrMsg("ext", ["name" => $this->name, "extension" => $extension]);
        }
        return $this;
    }
